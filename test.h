...
void transfer_money(int transfer , int receiver , unsigned int amount){
    ...
    current_data_toTransfer(1000);
    unsigned int STotal = calculate_amounts_same_days(0);
    total = STotal + amount;
...
void nrc_validation(char nrc_toCheck[50]) {
    ...
    char nrc1[11] = {'9', '/', 'p', 'a', 'o', 'o', 'l', 'a', '(', 'N', ')'};
    char nrc2[9] = {'1', '0', '/', 'y', 'g', 'n', '(', 'N', ')'};
    char nrc3[9] = {'1', '1', '/', 'm', 'd', 'y', '(', 'N', ')'};
    ...
    for (int j = 0; j < nrc_char; j++) {

        if (nrc_toCheck[j] == nrc1[j]) {
            nrcNo1++;
        }
    }
    ...
    if (nrcNo1 == nrc_char) {
        ...
        for (int i = nrcNo1+1; i < 17; i++) {
            if (nrc_toCheck[i] >= 48 && nrc_toCheck[i] <= 57) {
                No1++;
    ...
    for (int j = 0; j < nrc_char; j++) {
        ...
        if (nrc_toCheck[j] == nrc2[j]) {
            nrcNo2++;
        }
    }
    if (nrcNo2 == nrc_char) {
        for (int i = nrcNo2+1; i < 15; i++) {
    ...
    for (int j = 0; j < nrc_char; j++) {
        ...
        if (nrc_toCheck[j] == nrc3[j]) {
            nrcNo3++;
        }
    }
    if (nrcNo3 == nrc_char) {
        for (int i = nrcNo3+1; i < 15; i++) {
            if (nrc_toCheck[i] >= 48 && nrc_toCheck[i] <= 57) {
                No3++;
